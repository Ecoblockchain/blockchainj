...
import org.blockchainj.script.ScriptChunk;
import static org.blockchainj.script.ScriptOpCodes.getOpCodeName;
import org.blockchainj.signers.LocalTransactionSigner;
import org.blockchainj.signers.TransactionSigner.ProposedTransaction;
...
public class GenerateLowSTests {
    ...
    public static void main(final String[] argv) throws NoSuchAlgorithmException, IOException {
        ...
        final TransactionSignature signature = TransactionSignature.decodeFromBlockchain(scriptSig.getChunks().get(0).data, true, false);
        ...
        System.out.println("[[[\""
            ...
            + "\"" + Utils.HEX.encode(proposedTransaction.partialTx.unsafeBlockchainSerialize()) + "\", \""
        ...
        input.setScriptSig(new ScriptBuilder().data(highSig.encodeToBlockchain()).data(scriptSig.getChunks().get(1).data).build());
        input.getScriptSig().correctlySpends(outputTransaction, 0, output.getScriptPubKey(),
