/*
 * Copyright 2014 The blockchainj team
 *
...
/** Takes a URL or blockchain URI and prints information about the payment request. */
public class PaymentProtocolTool {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Provide a blockchain URI or URL as the argument.");
            return;
        }
        dump(args[0]);
    }
    ...
    private static void dump(String arg) {
        try {
            ...
            } else if ("blockchain".equals(uri.getScheme())) {
                BlockchainURI bcuri = new BlockchainURI(arg);
                final String paymentRequestUrl = bcuri.getPaymentRequestUrl();
                if (paymentRequestUrl == null) {
                    System.err.println("No r= param in blockchain URI");
                    return;
                }
                session = PaymentSession.createFromBlockchainUri(bcuri).get();
            } else {
            ...
            StringBuilder output = new StringBuilder(
                    format("Blockchain payment request, version %d%nDate: %s%n", version, session.getDate()));
