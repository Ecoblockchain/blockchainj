...
import org.blockchainj.protocols.payments.PaymentProtocol;
import org.blockchainj.protocols.payments.PaymentProtocolException;
import org.blockchainj.protocols.payments.PaymentSession;
import org.blockchainj.script.ScriptBuilder;
import org.blockchainj.store.*;
import org.blockchainj.uri.BlockchainURI;
import org.blockchainj.uri.BlockchainURIParseException;
import org.blockchainj.utils.BriefLogFormatter;
import org.blockchainj.wallet.DeterministicSeed;
import org.blockchainj.wallet.DeterministicUpgradeRequiredException;
import org.blockchainj.wallet.DeterministicUpgradeRequiresPassword;
...
import org.blockchainj.core.listeners.BlocksDownloadedEventListener;
import org.blockchainj.core.listeners.DownloadProgressTracker;
import org.blockchainj.wallet.MarriedKeyChain;
import org.blockchainj.wallet.Protos;
import org.blockchainj.wallet.SendRequest;
import org.blockchainj.wallet.Wallet;
import org.blockchainj.wallet.WalletExtension;
import org.blockchainj.wallet.WalletProtobufSerializer;
...
import static org.blockchainj.core.Coin.parseCoin;
import static com.google.common.base.Preconditions.checkNotNull;

/**
 * A command line tool for manipulating wallets and working with Blockchain.
 */
public class WalletTool {
    ...
    private static org.blockchain.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchBlockchains(Coin comparison) {
            try {
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("blockchain")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    BlockchainURI paymentRequestURI = new BlockchainURI(location);
                    future = PaymentSession.createFromBlockchainUri(paymentRequestURI, verifyPki);
                }
                ...
                System.err.println("Invalid blockchain uri: " + e.getMessage());
                System.exit(1);
            } catch (InterruptedException e) {
                // Ignore.
            } catch (ExecutionException e) {
            ...
            try {
                paymentRequest = org.blockchain.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchBlockchains(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Blockchain address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
    ...
    static synchronized void onChange(final CountDownLatch latch) {
        ...
        if (condition.matchBlockchains(balance)) {
