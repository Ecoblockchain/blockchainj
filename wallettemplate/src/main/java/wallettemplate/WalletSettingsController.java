...
import org.blockchainj.crypto.MnemonicCode;
...
public class WalletSettingsController {
    ...
    public void initialize(@Nullable KeyParameter aesKey) {
        DeterministicSeed seed = Main.blockchain.wallet().getKeyChainSeed();
        if (aesKey == null) {
            if (seed.isEncrypted()) {
            ...
            this.aesKey = aesKey;
            seed = seed.decrypt(checkNotNull(Main.blockchain.wallet().getKeyCrypter()), "", aesKey);
    ...
    public void restoreClicked(ActionEvent event) {
        ...
        if (Main.blockchain.wallet().getBalance().value > 0) {
        ...
        informationalAlert("Wallet restore in progress",
                "Your wallet will now be resynced from the Blockchain network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down blockchainj and restart it with the new seed.
        Main.blockchain.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.blockchain.startAsync();
            }
        }, Platform::runLater);
        Main.blockchain.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.blockchain.wallet().decrypt(aesKey);
