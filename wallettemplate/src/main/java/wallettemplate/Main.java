...
public class Main extends Application {
    ...
    public static WalletAppKit blockchain;
    ...
    private void realStart(Stage mainWindow) throws IOException {
        ...
        // Tell blockchainj to run event handlers on the JavaFX UI thread. This keeps things simple and means
        ...
        if (blockchain.isChainFileLocked()) {
        ...
        blockchain.addListener(new Service.Listener() {
            @Override
            public void failed(Service.State from, Throwable failure) {
                GuiUtils.crashAlert(failure);
            }
        }, Platform::runLater);
        blockchain.startAsync();

        scene.getAccelerators().put(KeyCombination.valueOf("Shortcut+F"), () -> blockchain.peerGroup().getDownloadPeer().close());
    }

    public void setupWalletKit(@Nullable DeterministicSeed seed) {
        // If seed is non-null it means we are restoring from backup.
        blockchain = new WalletAppKit(params, new File("."), WALLET_FILE_NAME) {
            @Override
            protected void onSetupCompleted() {
                // Don't make the user wait for confirmations for now, as the intention is they're sending it
                // their own money!
                blockchain.wallet().allowSpendingUnconfirmedTransactions();
                Platform.runLater(controller::onBlockchainSetup);
            }
        };
        // Now configure and start the appkit. This will take a second or two - we could show a temporary splash screen
        // or progress widget to keep the user engaged whilst we initialise, but we don't.
        if (params == RegTestParams.get()) {
            blockchain.connectToLocalHost();   // You should run a regtest mode blockchaind locally.
        } else if (params == TestNet3Params.get()) {
            // As an example!
            blockchain.useTor();
            // blockchain.setDiscovery(new HttpDiscovery(params, URI.create("http://localhost:8080/peers"), ECKey.fromPublicOnly(BaseEncoding.base16().decode("02cba68cfd0679d10b186288b75a59f9132b1b3e222f6332717cb8c4eb2040f940".toUpperCase()))));
        }
        blockchain.setDownloadListener(controller.progressBarUpdater())
               .setBlockingStartup(false)
               .setUserAgent(APP_NAME, "1.0");
        if (seed != null)
            blockchain.restoreWalletFromSeed(seed);
    }
    ...
    public void stop() throws Exception {
        blockchain.stopAsync();
        blockchain.awaitTerminated();
