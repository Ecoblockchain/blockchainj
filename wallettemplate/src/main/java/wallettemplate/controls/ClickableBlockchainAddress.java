...
/**
 * A custom control that implements a clickable, copyable Blockchain address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableBlockchainAddress extends AnchorPane {
    ...
    public ClickableBlockchainAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("blockchain_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return BlockchainURI.convertToBlockchainURI(address.get(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableBlockchainAddress> overlay = Main.instance.overlayUI(pane, this);
