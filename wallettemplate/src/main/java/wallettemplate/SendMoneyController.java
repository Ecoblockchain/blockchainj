...
import wallettemplate.controls.BlockchainAddressValidator;
...
public class SendMoneyController {
    ...
    public void initialize() {
        Coin balance = Main.blockchain.wallet().getBalance();
        checkState(!balance.isZero());
        new BlockchainAddressValidator(Main.params, address, sendBtn);
    ...
    public void send(ActionEvent event) {
        ...
        try {
            ...
            SendRequest req;
            if (amount.equals(Main.blockchain.wallet().getBalance()))
                req = SendRequest.emptyWallet(destination);
            else
                req = SendRequest.to(destination, amount);
            req.aesKey = aesKey;
            sendResult = Main.blockchain.wallet().sendCoins(req);
            ...
            ((HBox)BCHLabel.getParent()).getChildren().remove(BCHLabel);
            updateTitleForBroadcast();
