...
import org.blockchainj.core.listeners.TransactionConfidenceEventListener;
...
import org.blockchainj.core.PeerAddress;
...
import org.blockchainj.core.TransactionConfidence.ConfidenceType;
import org.blockchainj.crypto.*;
import org.blockchainj.script.Script;
import org.blockchainj.script.ScriptBuilder;
import org.blockchainj.signers.StatelessTransactionSigner;
import org.blockchainj.signers.TransactionSigner;
import org.blockchainj.store.BlockStoreException;
import org.blockchainj.store.MemoryBlockStore;
import org.blockchainj.testing.*;
import org.blockchainj.utils.ExchangeRate;
import org.blockchainj.utils.Fiat;
import org.blockchainj.utils.Threading;
import org.blockchainj.wallet.Wallet.BalanceType;
import org.blockchainj.wallet.WalletTransaction.Pool;
...
import org.blockchainj.wallet.Protos.Wallet.EncryptionType;
...
import static org.blockchainj.core.Utils.HEX;
import static org.blockchainj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the Blockchain protocol makes it difficult to determine the fee of an
    ...
    // identical result to Blockchain Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove Blockchain Core would accept it .... no such luck unfortunately.
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(OTHER_ADDRESS, valueOf(0, 10));
        // Reserialize.
        Transaction send2 = PARAMS.getDefaultSerializer().makeTransaction(send1.blockchainSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = PARAMS.getDefaultSerializer().makeTransaction(tx.blockchainSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = PARAMS.getDefaultSerializer().makeTransaction(send2.blockchainSerialize());
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.blockchainSerialize();
        buf[43] = 0;  // Break the signature: blockchainj won't check in SPV mode and this is easier than other mutations.
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = PARAMS.getDefaultSerializer().makeTransaction(t1.blockchainSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(AbstractBlockChain.NewBlockType.BEST_CHAIN, outbound1);
        // We should have a balance of 1 BCH after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeBlockchainSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeBlockchainSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.unsafeBlockchainSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.unsafeBlockchainSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("blockchainj-unit-test", null);
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers BCH, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Transaction tx = new Transaction(PARAMS);
        Coin messagePrice = Coin.ZERO;
        Script script = ScriptBuilder.createOpReturnScript("hello world!".getBytes());
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertTrue(request15.tx.unsafeBlockchainSerialize().length > 1000);
        ...
        assertTrue(request16.tx.unsafeBlockchainSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.unsafeBlockchainSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.unsafeBlockchainSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.unsafeBlockchainSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.unsafeBlockchainSerialize().length >= 998);
        assertTrue(spend18.unsafeBlockchainSerialize().length <= 1001);
        ...
        request26.tx.addOutput(CENT.subtract(
                Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.add(Transaction.MIN_NONDUST_OUTPUT).subtract(SATOSHI)),
                OTHER_ADDRESS);
        assertTrue(request26.tx.unsafeBlockchainSerialize().length > 1000);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        request4.feePerKb = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.divide(request3.tx.unsafeBlockchainSerialize().length);
        request4.ensureMinRequiredFee = true;
        wallet.completeTx(request4);
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToBlockchain();
    ...
    public void totalReceivedSent() throws Exception {
        // Receive 4 BCH in 2 separate transactions
        ...
        // Send 3 BCH in a single transaction
