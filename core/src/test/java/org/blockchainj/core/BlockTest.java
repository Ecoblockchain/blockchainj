...
import org.blockchainj.core.AbstractBlockChain.NewBlockType;
...
public class BlockTest {
    ...
    public void testWork() throws Exception {
        ...
        // This number is printed by Blockchain Core at startup as the calculated value of chainWork on testnet:
        //
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = PARAMS.getDefaultSerializer().makeBlock(header.blockchainSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testBlockchainSerialization() throws Exception {
        ...
        //
        // NB: This tests the blockchain serialization protocol.
        Block block = PARAMS.getDefaultSerializer().makeBlock(blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.blockchainSerialize()));
    }
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.blockchainSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.blockchainSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.unsafeBlockchainSerialize().length, tx.length);
        assertEquals(origTxLength, tx.length);
        block.addTransaction(tx);
        assertEquals(block.unsafeBlockchainSerialize().length, block.length);
        ...
        assertEquals(block.length, block.unsafeBlockchainSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
