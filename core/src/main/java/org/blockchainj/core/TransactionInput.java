...
import org.blockchainj.wallet.DefaultRiskAnalysis;
import org.blockchainj.wallet.KeyBag;
import org.blockchainj.wallet.RedeemData;
...
public class TransactionInput extends ChildMessage {
    ...
    protected void blockchainSerializeToStream(OutputStream stream) throws IOException {
        outpoint.blockchainSerialize(stream);
     ...
     * "from address" is not well defined in Blockchain and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Blockchain wiki for
     ...
     * @see TransactionOutPoint#getConnectedRedeemData(org.blockchainj.wallet.KeyBag)
     */
    @Nullable
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, blockchainSerialize(), 0);
    }

    /**
     * <p>Returns either RuleViolation.NONE if the input is standard, or which rule makes it non-standard if so.
     * The "IsStandard" rules control whether the default Blockchain Core client blocks relay of a tx / refuses to mine it,
