...
import org.blockchainj.net.StreamConnection;
import org.blockchainj.store.BlockStore;
import org.blockchainj.store.BlockStoreException;
import org.blockchainj.utils.ListenerRegistration;
...
/**
 * <p>A Peer handles the high level communication with a Blockchain node, extending a {@link PeerSocketHandler} which
 * handles low-level message (de)serialization.</p>
 *
 * <p>Note that timeouts are handled by the extended
 * {@link org.blockchainj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.blockchainj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.blockchainj.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.blockchainj.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.blockchainj.core.TxConfidenceTable}
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // blockchainj is a client mode implementation. That means there's not much point in us talking to other client
    ...
    protected void processAlert(AlertMessage m) {
        try {
            ...
            // BigInteger implementations! See https://github.com/blockchainj/blockchainj/issues/526 for discussion. As
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in Blockchain Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 Blockchain Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But Blockchain Core may change in future so
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.blockchain.it/wiki/Protocol_specification#getblocks
        //
     ...
     * Once the pong is received the value returned by {@link org.blockchainj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.blockchainj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.blockchainj.core.Peer#ping()} has never
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to Blockchain XT
     ...
     * <b>Most peers do not support this request. You will need to connect to Blockchain XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find Blockchain XT nodes.");
