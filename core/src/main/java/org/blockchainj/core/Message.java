...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using the Blockchain serialization format.
...
public abstract class Message {
     ...
     * @param protocolVersion Blockchain protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = blockchainSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Blockchain protocol.
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeBlockchainSerialize()}, which is safe to mutate.
    ...
    public byte[] blockchainSerialize() {
        byte[] bytes = unsafeBlockchainSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the blockchain wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeBlockchainSerialize() {
        ...
        try {
            blockchainSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (serializer.isParseRetainMode()) {
            ...
            // which means another call to blockchainSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the blockchain wire format.
     *
     * @param stream
     * @throws IOException
     */
    public final void blockchainSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use blockchainSerialize().
     */
    protected void blockchainSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented blockchainSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so BlockchainSerializer can avoid 2 instanceof checks + a casting.
     */
