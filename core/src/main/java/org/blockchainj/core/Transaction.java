...
import org.blockchainj.crypto.TransactionSignature;
import org.blockchainj.script.Script;
import org.blockchainj.script.ScriptBuilder;
import org.blockchainj.script.ScriptOpCodes;
...
import static org.blockchainj.core.Utils.*;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Blockchain C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Blockchain and have many powerful features. Read
 * <a href="https://blockchainj.github.io/working-with-transactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All Blockchain transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage {
    ...
    /**
     * If feePerKb is lower than this, Blockchain Core will treat it as if there were no fee.
     */
    public static final Coin REFERENCE_DEFAULT_MIN_TX_FEE = Coin.valueOf(5000); // 0.05 mBCH

    /**
     * If using this feePerKb, transactions will get confirmed within the next couple of blocks.
     * This should be adjusted from time to time. Last adjustment: March 2016.
     */
    public static final Coin DEFAULT_TX_FEE = Coin.valueOf(50000); // 0.5 mBCH
    ...
    // These are blockchain serialized.
    ...
    public Sha256Hash getHash() {
        if (hash == null) {
            hash = Sha256Hash.wrapReversed(Sha256Hash.hashTwice(unsafeBlockchainSerialize()));
        }
        return hash;
    }

    /**
     * Used by BlockchainSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Blockchain share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain) {
        ...
        if (fee != null) {
            final int size = unsafeBlockchainSerialize().length;
    ...
    public void clearInputs() {
        ...
        this.length = this.unsafeBlockchainSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Blockchain scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.blockchainj.script.Script, ECKey, org.blockchainj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], org.blockchainj.core.Transaction.SigHash, boolean)}
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.blockchain.it/wiki/Contracts

        try {
            // Create a copy of this transaction to operate upon because we need make changes to the inputs and outputs.
            // It would not be thread-safe to change the attributes of the transaction object itself.
            Transaction tx = this.params.getDefaultSerializer().makeTransaction(this.blockchainSerialize());
            ...
            // This step has no purpose beyond being synchronized with Blockchain Core's bugs. OP_CODESEPARATOR
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Blockchain 0.1.
            ...
            // Set the input to the script of its output. Blockchain Core does this but the step has no obvious purpose as
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                if (inputIndex >= tx.outputs.size()) {
                    ...
                    // Blockchain implementation. Blockchain Core also contains a bug in handling this case:
                    // any transaction output that is signed in this case will result in both the signed output
                    // and any future outputs to this public key being steal-able by anyone who has
                    // the resulting signature and the public key (both of which are part of the signed tx input).

                    // Blockchain Core's bug is that SignatureHash was supposed to return a hash and on this codepath it
    ...
    @Override
    protected void blockchainSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.blockchainSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.blockchainSerialize(stream);
     ...
     * since Blockchain 0.8+ a transaction that did not end its lock period (non final) is considered to be non
    ...
    /**
     * Returns the confidence object for this transaction from the {@link org.blockchainj.core.TxConfidenceTable}
     ...
     * <a href="https://github.com/blockchain/bips/blob/master/bip-0125.mediawiki">full replace-by-fee </a> semantics.
     */
     ...
     * This is useful in certain types of <a href="http://en.blockchain.it/wiki/Contracts">contracts</a>, such as
     * micropayment channels.</p>
     *
     * <p>Note that currently the replacement feature is disabled in Blockchain Core and will need to be
