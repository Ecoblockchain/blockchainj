...
public class BloomFilter extends Message {
     ...
     * See {@link org.blockchainj.core.BloomFilter.BloomUpdate} for a brief description of each mode. The purpose
    ...
    /**
     * Returns true if this filter will match anything. See {@link org.blockchainj.core.BloomFilter#setMatchAll()}
    ...
    public synchronized boolean applyAndUpdate(Transaction tx) {
            ...
            for (ScriptChunk chunk : script.getChunks()) {
                ...
                if (contains(chunk.data)) {
                    ...
                    if (flag == BloomUpdate.UPDATE_ALL || (flag == BloomUpdate.UPDATE_P2PUBKEY_ONLY && isSendingToPubKeys))
                        insert(output.getOutPointFor().unsafeBlockchainSerialize());
                    found = true;
                }
            }
        }
        ...
        for (TransactionInput input : tx.getInputs()) {
            if (contains(input.getOutpoint().unsafeBlockchainSerialize())) {
                return true;
            }
