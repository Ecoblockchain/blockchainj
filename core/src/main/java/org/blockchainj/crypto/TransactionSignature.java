...
package org.blockchainj.crypto;
...
/**
 * A TransactionSignature wraps an {@link org.blockchainj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * Because Blockchain Core works via bit testing, we must not lose the exact value when round-tripping
     ...
     * Blockchain Core. DER and the SIGHASH encoding allow for quite some flexibility in how the same structures
     ...
     * and generally violates people's mental model of how Blockchain should work, thus, non-canonical signatures are now
     * not relayed by default.
     */
    public static boolean isEncodingCanonical(byte[] signature) {
        // See Blockchain Core's IsCanonicalSignature, https://blockchaintalk.org/index.php?topic=8392.msg127623#msg127623
     ...
     * of the type used by Blockchain we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToBlockchain() {
        try {
     ...
     * @deprecated use {@link #decodeFromBlockchain(byte[], boolean, boolean} instead}.
     */
    @Deprecated
    public static TransactionSignature decodeFromBlockchain(byte[] bytes,
                                                         boolean requireCanonicalEncoding) throws VerificationException {
        return decodeFromBlockchain(bytes, requireCanonicalEncoding, false);
    }
    ...
    public static TransactionSignature decodeFromBlockchain(byte[] bytes,
        ...
        // Blockchain encoding is DER signature + sighash byte.
        ...
        // In Blockchain, any value of the final byte is valid, but not necessarily canonical. See javadocs for
