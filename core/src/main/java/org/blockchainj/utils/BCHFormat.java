...
import org.blockchainj.utils.BCHAutoFormat.Style;
import static org.blockchainj.utils.BCHAutoFormat.Style.*;
 ...
 * representations of Blockchain monetary values.
 *
 ...
 * represents a Blockchain monetary value.  Some of the features offered for doing this
 * are: <ol>
 ...
 *       {@link BCHFormat#format(Object)} method. 
 *   <li>Parse a value by passing a <code>String</code>-type
 *       representation of it to the {@link BCHFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:
 *
 * <blockquote><pre>
 * BCHFormat f = BCHFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "BCH 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "BCH 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mBCH 1.00"</strong>
 ...
 * be formatted, {@link BCHAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link BCHFixedFormat}.  
 *
 ...
 * values of {@link BCHAutoFormat.Style}.  There are two styles constants: {@link
 * BCHAutoFormat.Style#CODE} (the default), and {@link BCHAutoFormat.Style#SYMBOL}.  The
 * difference is that the <code>CODE</code> style uses an internationally-distinct currency
 * code, such as <code>"BCH"</code>, to indicate the units of denomination, while the
 * <code>SYMBOL</code> style uses a possibly-ambiguous currency symbol such as
 * <code>"฿"</code>.
 *
 * <p>The denomination used when formatting will be either blockchain, millicoin
 ...
 * locale, a value of one blockchain might be formatted as <pre>฿1.00</pre> where a value
 ...
 * increasing precision to convert the representation of a given quantity of blockchains into a
 * representation of the same value denominated in the formatter's units.  For example, a scale
 * value of <code>3</code> specifies a denomination of milliblockchains, because to represent
 * <code>1.0000 BCH</code>, or one blockchain, in milliblockchains, one shifts the decimal point
 * three places, that is, to <code>1000.0 mBCH</code>.
 *
 * <h3>Construction</h3>
 *
 * There are two ways to obtain an instance of this class: <ol>
 *   <li>Use one of the factory methods; or
 *   <li>Use a {@link BCHFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class <code>BCHFormat</code>.  There
 ...
 * automatic-denominating <code>BCHAutoFormat</code> instance for your
 ...
 * <blockquote><pre>
 * BCHFormat f = BCHFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "BCH 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link BCHFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of microblockchains:
 *
 * <blockquote><pre>BCHFormat m = BCHFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>BCHFormat milliFormat = BCHFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * <code>getInstance()</code> is one of the enumerated values of the
 * {@link BCHAutoFormat.Style} type, either <code>CODE</code> or
 * <code>SYMBOL</code>, then you will get a {@link BCHAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.
 *
 * <blockquote><pre>
 * BCHFormat s = BCHFormat.getInstance(SYMBOL);
 ...
 * differently the same one-blockchain value:
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 BCH"</strong>
 * BCHFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 ฿"</strong>
 * BCHFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * BCHFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * BCHFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link BCHFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a <code>BCHFormat</code>
 * object.  Its {@link Builder#build()} method constructs and returns a <code>BCHFormat</code> instance
 ...
 *
 * <blockquote><pre>
 * BCHFormat f = BCHFormat.builder().style(CODE).code("XBT").build();
 * String out = f.format(COIN); <strong>// "XBT 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link BCHFormat.Builder} class for details.
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Blockchain monetary value by passing it to the {@link BCHFormat#format(Object)}
 * method.  This argument can be either a {@link org.blockchainj.core.Coin}-type object or a
 ...
 * number of blockchains.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link BCHFixedFormat} also accept a variable-length sequence of additional <code>int</code>
 ...
 *
 * <blockquote><pre>
 * BCHFormat f = BCHFormat.getCoinInstance();
 ...
 * {@link BCHFixedFormat#REPEATING_PLACES}, {@link
 * BCHFixedFormat#REPEATING_DOUBLETS} and {@link
 * BCHFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * BCHFormat de = BCHFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µBCH"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µBCH"</strong>
 ...
 *
 * <blockquote><pre>
 * BCHFixedFormat kilo = (BCHFixedFormat)BCHFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kBCH 1.23"</strong>
 ...
 * BCHFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * BCHFormat f = BCHFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * BCHFixedFormat} and {@link BCHAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link BCHAutoFormat} recognize
 ...
 * instances of {@link BCHFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * BCHFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link BCHFormat.Builder#pattern()} method.
 *
 * <p>The {@link BCHAutoFormat#parse(String)}</code> method of {@link BCHAutoFormat} (and of
 * {@link BCHAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * <code>µBCH</code> or other appropriate permutations of those characters.  Additionally, if
 * either or both of a custom currency code or symbol is configured using {@link
 * BCHFormat.Builder#code} or {@link BCHFormat.Builder#code}, then such code or symbol will
 ...
 * <code>BCHFormat</code> instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * BCHFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed <code>String</code>s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link BCHAutoFormat}
 * instance will infer a denomination of blockchains while a {@link BCHFixedFormat} will infer the
 ...
 * @see          org.blockchainj.core.Coin
 */

public abstract class BCHFormat extends Format {
    ...
    /** The conventional international currency code for blockchains: "BCH" */
    private static final String COIN_CODE = "BCH";
    /** The default currency symbols for blockchains */
    ...
    /* Scale is the number of decimal-places difference from same value in blockchains */
    /** A constant useful for specifying a denomination of blockchains, the <code>int</code> value
     *  <code>0</code>. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of milliblockchains, the <code>int</code>
     *  value <code>3</code>. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of microblockchains, the <code>int</code>
     *  value <code>6</code>. */
    ...
    /**
     * <p>This class constructs new instances of {@link BCHFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * <code>Builder</code> object from the {@link BCHFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new <code>BCHFormat</code> object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking <code>build()</code> without invoking any
     * of the setting methods is equivalent to invoking {@link BCHFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override BCHFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            BCHFormat newInstance(Builder b) {
        ...
        private Style style = BCHAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new <code>BCHFormat</code> is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new <code>BCHFormat</code>
        ...
        public Builder style(BCHAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link BCHAutoFormat} instances.
         *
         ...
         *  denominated in blockchains.  For example, a denomination of milliblockchains is specified
         *  with a value of <code>3</code>.
         *
         * <p>The <code>BCHFormat</code> class provides appropriately named
         * <code>int</code>-type constants for the three common values, {@link BCHFormat#COIN_SCALE},
         * {@link BCHFormat#MILLICOIN_SCALE} {@link BCHFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(BCHAutoFormat.Style)} is invoked on a
         * <code>Builder</code>, then the <code>BCHFormat</code> will default to a
         * fixed-denomination of blockchains, equivalent to invoking this method with an argument
         * of <code>0</code>. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the <code>BCHFormat</code>
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link BCHFormat#symbols()}.
        ...
        /** Return a new {@link BCHFormat} instance.  The object returned will be configured according
         *  to the state of this <code>Builder</code> instance at the time this method is invoked. */
        public BCHFormat build() {
            BCHFormat f = variant.newInstance(this);
    ...
    protected BCHFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "<code>BCH</code>", to indicate that denomination.  The
     ...
     */
    public static BCHFormat getInstance() { return getInstance(defaultLocale()); }
     ...
     */
    public static BCHFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, <code>"BCH"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static BCHFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * locale.
     */
    public static BCHFormat getSymbolInstance(int fractionPlaces) {
     ...
     * <code>"BCH"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static BCHFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, <code>"mBCH"</code>.
     */
    public static BCHFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, <code>"mBCH"</code>.
     */
    public static BCHFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * <code>"mBCH"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static BCHFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static BCHFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     */
    public static BCHFormat getSymbolInstance(Locale locale) {
     ...
     * fractional satoshis.
     */
    public static BCHFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     * <code>"฿"</code>, or code, such as <code>"mBCH"</code>, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static BCHFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     * <code>"mBCH"</code>, depending on the value of the first argument.  The returned object
     ...
     */
    public static BCHFormat getInstance(Style style, int fractionPlaces) {
     ...
     * "<code>BCH</code>", or symbol, such as "<code>฿</code>", depending on the value
     ...
     */
    public static BCHFormat getInstance(Style style, Locale locale) {
     ...
     * such as <code>"mBCH"</code>, or symbol, such as "<code>฿</code>",
     ...
     * fractional satoshis.
     */
    public static BCHFormat getInstance(Style style, Locale locale, int fractionPlaces) {
	return new BCHAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static BCHFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static BCHFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static BCHFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of blockchains, with the given minimum number of fractional
     ...
     */
    public static BCHFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static BCHFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static BCHFormat getMilliInstance(Locale locale) {
     ...
     */
    public static BCHFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static BCHFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static BCHFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static BCHFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static BCHFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static BCHFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static BCHFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static BCHFormat getInstance(int scale) {
     ...
     */
    public static BCHFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static BCHFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static BCHFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new BCHFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
     * Formats a blockchain monetary value and returns an {@link java.text.AttributedCharacterIterator}.
    ...
    /**
     * Formats a blockchain value as a number and possibly a units indicator and appends the
     ...
     * number of blockchains, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a blockchain value as a number and possibly a units indicator to a
     ...
     * <code>BigDecimal</code> is interpreted as that number of blockchains, rounded to the
     ...
     * is the size of the decimal-place shift between the given Blockchain-value denominated in
     * blockchains and that same value as formatted.  A fixed-denomination formatter will ignore
     * the arguments.
     *
    ...
    /**
     * Takes a blockchain monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a blockchain quantity of any type the
	...
	BigInteger satoshis;
        // the value might be blockchains or satoshis
    ...
    /**
      * Parse a <code>String</code> representation of a Blockchain monetary value.  Returns a
      * {@link org.blockchainj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
    ...
    /** Parse a <code>String</code> representation of a Blockchain monetary value.  If this
     * object's pattern includes a currency sign, either symbol or code, as by default is true
     * for instances of {@link BCHAutoFormat} and false for instances of {@link
     * BCHFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * String, then the number is interpreted as a quatity of blockchains.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * BCHAutoFormat} will interpret the parsed number as a quantity of blockchains, and instances
     * of {@link BCHAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * BCHFixedFormat#symbol()} or {@link BCHFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for blockchains configured for this object.  The
    ...
    /** Return the unprefixed international currency code for blockchains configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof BCHFormat)) return false;
        BCHFormat other = (BCHFormat)o;
