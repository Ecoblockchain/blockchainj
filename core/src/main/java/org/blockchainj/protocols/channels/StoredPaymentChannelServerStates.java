...
public class StoredPaymentChannelServerStates implements WalletExtension {
     ...
     * is measured in terms of our local clock, and the refund transaction's lock time is measured in terms of Blockchain
     ...
     * This setter will let you delay the setting of the broadcaster until the Blockchain network is ready.
     *
        ...
        try {
            ...
            for (StoredServerChannel channel : mapChannels.values()) {
                ...
                ServerState.StoredServerPaymentChannel.Builder channelBuilder = ServerState.StoredServerPaymentChannel.newBuilder()
                        ...
                        .setContractTransaction(ByteString.copyFrom(channel.contract.unsafeBlockchainSerialize()))
                        .setMyKey(ByteString.copyFrom(channel.myKey.getPrivKeyBytes()));
                if (channel.majorVersion == 1) {
                    channelBuilder.setClientOutput(ByteString.copyFrom(channel.clientOutput.unsafeBlockchainSerialize()));
                } else {
