...
import org.blockchainj.core.AddressFormatException;
...
/**
 * <p>Provides a standard implementation of a Blockchain URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code blockchain:<address>}</li>
 * <li>{@code blockchain:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/blockchain/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class BlockchainURI {
    ...
    /**
     * URI for Blockchain network. Use {@link org.blockchainj.params.AbstractBlockchainNetParams#BLOCKCHAIN_SCHEME} if you specifically
     * need Blockchain, or use {@link org.blockchainj.core.NetworkParameters#getUriScheme} to get the scheme
    ...
    /**
     * Constructs a new BlockchainURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws BlockchainURIParseException if the URI is not syntactically or semantically valid.
     */
    public BlockchainURI(String uri) throws BlockchainURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Blockchain URI.
     *
     ...
     *
     * @throws BlockchainURIParseException If the input fails Blockchain URI syntax and semantic checks.
     */
    public BlockchainURI(@Nullable NetworkParameters params, String input) throws BlockchainURIParseException {
        checkNotNull(input);

        String scheme = null == params
            ? AbstractBlockchainNetParams.BLOCKCHAIN_SCHEME
            : params.getUriScheme();
        ...
        try {
            ...
            throw new BlockchainURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  blockchain:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form blockchain://address?....
        // We support both until Ben fixes his code.
        
        // Remove the blockchain scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : blockchain:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (input.startsWith(blockchainInfoScheme)) {
            ...
            throw new BlockchainURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new BlockchainURIParseException("No data found after the blockchain: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Blockchain address for this network
            try {
                Address address = Address.fromBase58(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new BlockchainURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new BlockchainURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws BlockchainURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new BlockchainURIParseException("Malformed Blockchain URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new BlockchainURIParseException("Malformed Blockchain URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new BlockchainURIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws BlockchainURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new BlockchainURIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Blockchain Address from the URI, if one was present. It's possible to have Blockchain URI's with no address if a
     ...
     *         10,000,000 units is 1 BCH. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("BlockchainURI[");
        boolean first = true;
    ...
    /**
     * Simple Blockchain URI builder using known good fields.
     *
     * @param address The Blockchain address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the Blockchain URI
     */
    public static String convertToBlockchainURI(Address address, Coin amount,
                                             String label, String message) {
        return convertToBlockchainURI(address.getParameters(), address.toString(), amount, label, message);
    }
     ...
     * @return A String containing the Blockchain URI
     */
    public static String convertToBlockchainURI(NetworkParameters params,
