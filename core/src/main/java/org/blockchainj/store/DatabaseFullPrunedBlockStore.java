...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See blockchainj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.blockchainj.core.NetworkParameters}.
    ...
    private void createNewStore(NetworkParameters params) throws BlockStoreException {
        try {
            ...
            // The coinbase in the genesis block is not spendable. This is because of how Blockchain Core inits
    ...
    protected void putUpdateStoredBlock(StoredBlock storedBlock, boolean wasUndoable) throws SQLException {
        try {
            ...
            s.setBytes(4, storedBlock.getHeader().cloneAsHeader().unsafeBlockchainSerialize());
     ...
     *
     * <p>The balance {@link org.blockchainj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
